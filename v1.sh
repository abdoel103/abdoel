...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://ns.beeplus.online/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://ns.beeplus.online/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://ns.beeplus.online/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "   '----------------------------------'"
echo -e "     \e[1;32m1)\e[0m Use Private Domain"
...
#install cf
wget -q ${REPO}cobek/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${REPO}kucik/config.json" >/dev/null 2>&1
    #wget -O /usr/local/bin/xray "${REPO}xray/xray.linux.64bit" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${REPO}kucik/runn.service" >/dev/null 2>&1
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPO}kucik/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPO}kucik/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    curl ${REPO}kucik/nginx.conf > /etc/nginx/nginx.conf
...
#SERVICE VMESS
# // Installing UDPGW
wget -O /usr/bin/badvpn "${REPO}tangkal/badvpn" >/dev/null 2>&1
...
print_install "Install modul SlowDNS Server"
    wget -q -O /tmp/nameserver "${REPO}linak/nameserver" >/dev/null 2>&1
...
wget -q -O /etc/ssh/sshd_config "${REPO}caw/sshd" >/dev/null 2>&1
...
#OpenVPN
wget ${REPO}caw/openvpn &&  chmod +x openvpn && ./openvpn
...
wget -q -O /etc/ipserver "${REPO}bantur/ipserver" && bash /etc/ipserver
...
gotop_latest="$(curl -s https://api.github.com/repos/xxxserxxx/gotop/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    ...
    wget ${REPO}cobek/bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
...
wget -O /usr/sbin/ftvpn "${REPO}kijuk/ftvpn" >/dev/null 2>&1
